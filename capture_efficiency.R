  ...
  moduleServer(id, 
      ...
      observeEvent(deploy$refresh_cet(), {
        if(deploy$refresh_cet() > 0){
          ...
          updateSelectizeInput(session, "system_id", selected = deploy$system_id())
        }
      })
      ...
      observeEvent(input$cet_table_rows_selected, {
        ...
        # updateSelectizeInput(session, "system_id", selected = rv$cet_table_db()[input$cet_table_rows_selected, 2])
