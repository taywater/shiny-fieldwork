...
documentationUI <- function(id, label = "documentation"){
  tabPanel("Documentation", value = "readme_tab", 
           ...
           column(width = 5,
                  ...
                  h5("The Future Deployments tab presents a table of all future, or planned, deployments. Click a row to go to the Deploy Sensor tab, where the same deployment should be selected and available to edit.", style = "margin-left: 20px"),
                  h3("Add Location"), 
                  h5("Choose whether to add a location at an SMP or non-SMP site (\"Site\"). If SMP: Select an SMP ID to give options for Component ID. A suggested Location (ie, OW1) is autofilled, which can be overwritten if needed. Facility ID is autofilled based on the selected Component ID. If the selected feature does not have a component ID, the facility ID of the SMP will be used. Once all field are in the top sidebar are complete, and the new location does not already exist, add the location. Click a row on the table to enable editing of location. Use the lower sidebar to add or edit well measurements for the location noted in the top sidebar. If Site: Select a Site in the dropdown. If the Site you need isn't there, you can add it by checking off \"Add New Site?\" and typing a new name. Once you click, it will appear in the dropdown. Select a component ID and location. If \"At SMP\" is blank, you can check a box if you want to move monitoring locations from a site to an SMP. This is useful if a monitored pre-construction site was given an SMP ID. The locations will be assigned to that SMP ID, and the site name will removed." , style = "margin-left: 20px"), 
                  h3("Add Sensor"), 
                  h5("Add a new sensor to the inventory by entering sensor serial number, model number, and sensor status. Purchase date is optional. Edit existing sensors by entering the sensor serial number, or clicking on the desired table row. The table can be sorted and searched. Click \"Deploy this Sensor\" to go to the \"Deploy Sensor\" tab with the sensor selected.", style = "margin-left: 20px"),
                  h3("Deploy Sensor"), 
